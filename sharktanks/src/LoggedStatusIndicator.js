...
class LoggedStatusIndicator extends Component {
  ...
  render() {
    ...
    const logOutButton = <RaisedButton label="Log Out" primary={true}
      onClick={() => {
        localLogOut()
    ...
    const logInButton = <Link to='/login'><RaisedButton label="Log In" primary={true} /> </Link>;

    const displayed = user ? 
       <div className={styles.sameLine}>
          <p className={styles.noMargin}>{user.name}</p> {logOutButton}{" "}
        </div> 
      : <div className={styles.sameLine}><h6>&nbsp;</h6>{logInButton}</div>;
